...
template <typename T = float>
__global__
void set_cpu_page_kernel(array_d_t<T>* dr, uint64_t *index_ptr, uint64_t page_size){
	bam_ptr<T> ptr(dr);
	if(threadIdx.x == 0){
		int64_t page_idx = index_ptr[blockIdx.x];
		ptr.set_cpu_page(page_idx * page_size, blockIdx.x * page_size);
	}
}
...
void BAM_Feature_Store::init_controllers(int ps, uint64_t read_off, uint64_t cache_size, uint64_t num_ele = 100, uint64_t num_ssd = 1, bool cpu_cache = false, uint64_t cpu_page_num = 0) {
  ...
  std::cout << "num ssd: " << num_ssd << std::endl;
  n_ctrls = num_ssd;
...
//  size_t cpu_cache_size = cpu_page_num * page_size;
  ...
  size_t cpu_cache_size = 1572864LL * page_size;
   
  TYPE* cpu_cache_ptr = nullptr; 
  if(cpu_cache){  
  	printf("cpu cache alloc\n");
	std::cout << "cpu page num2: " << cpu_page_num << std::endl;
	
  	  cudaMallocManaged((void**)(&cpu_cache_ptr), 4LL* cpu_cache_size * sizeof(TYPE));
   }
  a = new array_t<TYPE>(numElems, 0, vr, cudaDevice, cpu_cache, cpu_cache_ptr);
...
void BAM_Feature_Store::set_cpu_page(uint64_t cpu_idx, int64_t num_pages){

	uint64_t* cpu_idx_ptr = (uint64_t*) cpu_idx;
 	uint64_t page_size = pageSize;	
	set_cpu_page_kernel<TYPE><<<num_pages, 32>>>(a->d_array_ptr,cpu_idx_ptr, page_size);
	cuda_err_chk(cudaDeviceSynchronize())
}
...
PYBIND11_MODULE(BAM_Feature_Store, m) {
  ...
  py::class_<BAM_Feature_Store,
      ...
      .def("set_cpu_pages",&BAM_Feature_Store::set_cpu_page);
