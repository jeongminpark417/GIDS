...
def train_wrapper(rank, world_size, args):
    print("rank: ", rank, "start")
    ...
    track_acc(rank, world_size, g, args, labels)

def track_acc(rank, world_size, g, args, label_array=None):
    #print("rank: ", rank, "start")
    #init_process_group(world_size, rank)
    device = torch.device('cuda:{:d}'.format(rank))
    ...
    if(bam_flag):
        if(args.data == "IGB"):
            train_dataloader.dist_gids_init(0,4096,1024, 300*1000*1000*1024, args.cache_size, rank)
        elif(args.data == "OGB"):
            train_dataloader.dist_gids_init(0,128*4,128, 111059956 * 128 * 2,  args.cache_size, rank)
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        for idx, (input_nodes, seeds, blocks, ret) in enumerate(train_dataloader):
#            print("rank: ", rank, " idx: ", idx)
            if(idx == 1000 / world_size):
                print("warp up done")
...
if __name__ == '__main__':
    ...
    procs = []
#    mp.spawn(test_track_acc, args=( num_gpus, g, args, labels), nprocs=num_gpus)
    mp.spawn(train_wrapper, args=( num_gpus, args), nprocs=num_gpus)
 
  #  p = mp.Process(target=test_track_acc, args=(rank, num_gpus))
